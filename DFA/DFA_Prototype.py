...
t = torch.tensor([30.8930, 32.0070, 33.1470, 34.2900, 35.4100, 36.5270, 37.6670,
...
x = torch.tensor([1.1130, 1.1400, 1.1430, 1.1200, 1.1170, 1.1400, 1.1400, 1.1500, 1.1270,
...
def DFA_Method(x: torch.Tensor, t: torch.Tensor, window_size: int):
    ...
    fluctuation = torch.mean(sqrd_deviations, dim=1)
...
Windows_sizes = torch.logspace(start=log10(initial_window_size), end=log10(max_window_size), steps=75, base=10)
...
plt.plot(Windows_sizes_log, m * Windows_sizes_log + c, color=custom_colors["Red"], label="DFA", linewidth=0.75)
plt.grid()
