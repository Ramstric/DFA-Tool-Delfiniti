...
def DFA_Method(x: torch.Tensor, y: torch.Tensor, window_size: int | torch.Tensor, plot_epochs=False,
    ...
    RMS_deviation = torch.sqrt(fluctuation)
    ...
    for epoch in range(n_windows):
        ...
        plt.vlines(t_epochs[epoch][-1], torch.min(y_windows), torch.max(y_windows), color=color, linestyle=(5, (10, 3)), linewidth=0.25)
    """
    if check_remainder != 0 and remainder_numerate_windows.size(0) > 1:
...
def DFA_F_Plot(x: torch.Tensor, y: torch.Tensor, initial_window_size=10, window_size_step=45, plot_epochs=False, plot_time_series=False, plot_sum_series=False, save_path=None):
    """
    ...
    Windows_sizes = torch.logspace(start=log10(initial_window_size), end=log10(max_window_size), steps=window_size_step, base=10)
    ...
    if save_path:
        plt.savefig(save_path+"/DFA_plot.png")

    plt.close()
    #plt.show()

    if plot_time_series or plot_sum_series:
        _plot_series(x, y, plot_time_series, plot_sum_series, save_path)
...
def _plot_series(x: torch.Tensor, y: torch.Tensor, plot_time_series=True, plot_sum_series=False, save_path=None):
    ...
    if plot_time_series:
        plt.plot(x, y, color=custom_colors["Blue"], label="Time series", linewidth=0.75)

    if plot_sum_series:
        y_sum = torch.cumsum(y - y.mean(), dim=0)
        plt.plot(x, y_sum, color=custom_colors["Orange"], label="Integrated time series", linewidth=0.75)
        plt.scatter(x, y_sum, color=custom_colors["Orange"], s=2)

    plt.xlabel("Time")
    plt.ylabel("Amplitude")
    plt.legend()
    if save_path:
        plt.savefig(save_path+"/Time_series.png")
...
y_data = torch.tensor([1.1130, 1.1400, 1.1430, 1.1200, 1.1170, 1.1400, 1.1400, 1.1500, 1.1270,
                       ...
                       1.1430])
"""
#DFA_F_Plot(x_data, y_data, window_size_step=44)
